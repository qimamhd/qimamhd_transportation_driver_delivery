...
The server checks only destinations configured under the driver's source area, chooses the nearest valid destination, and requires the effective GPS distance to be within the `gps_radius` configured on the matched destination record.

Direct-delivery submission to `POST /api/driver/v1/deliveries` must include `submission_context=direct_delivery`. The server re-derives the driver's vehicle area and revalidates car, source, auto-matched destination, and that destination's configured GPS radius. Other delivery flows keep their existing behavior.
